...
def read_and_sort_words(filename):
    try:
        ...
        with open(filename, 'r', encoding='utf-8') as file:
            ...
            # # Розділити слова на українські та англійські, а потім відсортувати їх окремо
            # ukrainian_words_len = sorted([word for word in clean_words if is_ukrainian_word(word)], key=len)
            # english_words_len = sorted([word for word in clean_words if not is_ukrainian_word(word)], key=len)
